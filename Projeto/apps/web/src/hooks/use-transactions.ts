...
export function useTransactions(params?: ExtendedTransactionListParams) {
    ...
    const createTransaction = async (input: CreateTransactionInput) => {
        try {
            const newTransaction = await apiClient.post<Transaction>('/api/transactions', input)
    ...
    const updateTransaction = async (id: string, input: UpdateTransactionInput) => {
        try {
            const updatedTransaction = await apiClient.put<Transaction>(`/api/transactions/${id}`, input)
    ...
    // Carregar transações ao montar ou mudar params
    useEffect(() => {
        fetchTransactions()
    }, [fetchTransactions])
