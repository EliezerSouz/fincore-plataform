-- Migration: Evolve payment_methods to support business rules (FINAL)
-- Date: 2025-12-18
...
-- 2. Add business rule columns (one by one for better error handling)
DO $$ 
BEGIN
...
-- 3. Add new constraint with all valid types
ALTER TABLE payment_methods 
ADD CONSTRAINT payment_methods_type_check 
CHECK (type IN (
    'PIX', 'CASH', 'CREDIT_CARD', 'DEBIT_CARD', 
    'BANK_TRANSFER', 'BOLETO', 'AUTOMATIC', 
    'INTERNAL_TRANSFER', 'OTHER'
));

-- 4. Add comments for documentation
COMMENT ON COLUMN payment_methods.type IS 'Type category: PIX, CASH, CREDIT_CARD, DEBIT_CARD, BANK_TRANSFER, BOLETO, AUTOMATIC, INTERNAL_TRANSFER, OTHER';
...
-- 5. Create function to populate default payment methods for a user
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'PIX') THEN
        ...
        VALUES (p_user_id, 'PIX', 'pix', 'PIX', true, true, true, true, false, false, false, '⚡', 1, true);
    END IF;

    -- Dinheiro
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'CASH') THEN
        INSERT INTO payment_methods (user_id, name, slug, type, allows_income, allows_expense, allows_transfer, affects_balance, affects_credit_card, affects_invoice, is_internal, icon, sort_order, is_active)
        VALUES (p_user_id, 'Dinheiro', 'dinheiro', 'CASH', true, true, false, true, false, false, false, '💵', 2, true);
    END IF;

    -- Cartão de Crédito
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'CREDIT_CARD') THEN
        INSERT INTO payment_methods (user_id, name, slug, type, allows_income, allows_expense, allows_transfer, affects_balance, affects_credit_card, affects_invoice, is_internal, icon, sort_order, is_active)
        VALUES (p_user_id, 'Cartão de Crédito', 'cartao-credito', 'CREDIT_CARD', false, true, false, false, true, true, false, '💳', 3, true);
    END IF;

    -- Cartão de Débito
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'DEBIT_CARD') THEN
        INSERT INTO payment_methods (user_id, name, slug, type, allows_income, allows_expense, allows_transfer, affects_balance, affects_credit_card, affects_invoice, is_internal, icon, sort_order, is_active)
        VALUES (p_user_id, 'Cartão de Débito', 'cartao-debito', 'DEBIT_CARD', false, true, false, true, false, false, false, '💳', 4, true);
    END IF;

    -- Transferência Bancária
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'BANK_TRANSFER') THEN
        INSERT INTO payment_methods (user_id, name, slug, type, allows_income, allows_expense, allows_transfer, affects_balance, affects_credit_card, affects_invoice, is_internal, icon, sort_order, is_active)
        VALUES (p_user_id, 'Transferência Bancária', 'transferencia-bancaria', 'BANK_TRANSFER', true, true, true, true, false, false, false, '🏦', 5, true);
    END IF;

    -- Boleto
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'BOLETO') THEN
        INSERT INTO payment_methods (user_id, name, slug, type, allows_income, allows_expense, allows_transfer, affects_balance, affects_credit_card, affects_invoice, is_internal, icon, sort_order, is_active)
        VALUES (p_user_id, 'Boleto', 'boleto', 'BOLETO', true, true, false, true, false, false, false, '📄', 6, true);
    END IF;

    -- Débito Automático
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'AUTOMATIC') THEN
        INSERT INTO payment_methods (user_id, name, slug, type, allows_income, allows_expense, allows_transfer, affects_balance, affects_credit_card, affects_invoice, is_internal, icon, sort_order, is_active)
        VALUES (p_user_id, 'Débito Automático', 'debito-automatico', 'AUTOMATIC', false, true, false, true, false, false, false, '🔄', 7, true);
    END IF;

    -- Transferência Interna
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'INTERNAL_TRANSFER') THEN
        INSERT INTO payment_methods (user_id, name, slug, type, allows_income, allows_expense, allows_transfer, affects_balance, affects_credit_card, affects_invoice, is_internal, icon, sort_order, is_active)
        VALUES (p_user_id, 'Transferência Interna', 'transferencia-interna', 'INTERNAL_TRANSFER', false, false, true, false, false, false, true, '🔄', 8, true);
    END IF;

    -- Outro
    IF NOT EXISTS (SELECT 1 FROM payment_methods WHERE user_id = p_user_id AND type = 'OTHER') THEN
        INSERT INTO payment_methods (user_id, name, slug, type, allows_income, allows_expense, allows_transfer, affects_balance, affects_credit_card, affects_invoice, is_internal, icon, sort_order, is_active)
        VALUES (p_user_id, 'Outro', 'outro', 'OTHER', true, true, false, true, false, false, false, '📝', 99, true);
    END IF;
END;
$$ LANGUAGE plpgsql;

-- 6. Populate default payment methods for existing users
DO $$
DECLARE
    user_record RECORD;
BEGIN
...
-- 7. Create trigger to auto-create payment methods for new users (if auth.users exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auth' AND table_name = 'users') THEN
        -- Create function
...
BEGIN
    ...
    RAISE NOTICE '✅ Default payment methods created for all users.';
END $$;
